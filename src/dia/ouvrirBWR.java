...
public final class ouvrirBWR extends JDialog {
    ...
    /** Ouvre l’élément sélectionné : dossier → naviguer, fichier .bwr → charger. 
     * @throws Exception */
	    ...
	    if (list != null) {
	        for (File f : list) {
	            if (f.isDirectory()) {
	                ...
	                files++;
	                if (f.getName().toLowerCase().endsWith(".bwr")) bweCount++;
	            }
	        }
	    }

	    String msg = String.format(
	        "Dossier : %s%nDossier parent : %s%nFichiers .bwr : %d%nTotal : %d éléments (%d dossier(s), %d fichier(s))",
	...
	private void showSelectedFileInfo() {
	    ...
	    // Ne traiter que les fichiers .bwr
	    if (!f.isFile() || !f.getName().toLowerCase().endsWith(".bwr")) {
	        return;
	    }
