...
public final class ouvrirHTML extends JDialog {
...
//        });
	    ...
	    if (list != null) {
	        for (File f : list) {
	            if (f.isDirectory()) {
	                ...
	                files++;
	                if (f.getName().toLowerCase().endsWith(".html")) bweCount++;
	            }
	        }
	    }

	    String msg = String.format(
	        "Dossier : %s%nDossier parent : %s%nFichiers .html : %d%nTotal : %d éléments (%d dossier(s), %d fichier(s))",
	...
	private void showSelectedFileInfo() {
	    ...
	    // Ne traiter que les fichiers .html
