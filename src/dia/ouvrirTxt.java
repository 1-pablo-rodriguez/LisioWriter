...
public final class ouvrirTxt extends JDialog {
	...
	public ouvrirTxt(EditorFrame parent){
        super(parent);
		this.parent = parent;
...
//        setSize(1200, 800);                 // largeur x hauteur
    ...
    /** Ouvre l’élément sélectionné : dossier → naviguer, fichier .txt → charger. 
     * @throws Exception */
    private void openSelected() throws Exception {
        ...
        // Fichier .txt
        if (sel.isFile() && sel.getName().toLowerCase().endsWith(".txt")) {
        	new readFileTXT(sel,parent);
            closeDialog(false);
        }
    }
    ...
    // ——— Recherche incrémentale avec priorité .txt > fichiers > dossiers ———
	    ...
	    if (list != null) {
	        for (File f : list) {
	            if (f.isDirectory()) {
	                ...
	                files++;
	                if (f.getName().toLowerCase().endsWith(".txt")) bweCount++;
	            }
	        }
	    }

	    String msg = String.format(
	        "Dossier : %s%nDossier parent : %s%nFichiers .txt : %d%nTotal : %d éléments (%d dossier(s), %d fichier(s))",
