...
public final class ouvrirDOCX extends JDialog {
...
//        });
    ...
    private void loadDirectory(File dir) {
        ...
        if (list != null) {
            ...
            for (File f : list) {
                if (f.isDirectory() || f.getName().toLowerCase().endsWith(".docx")) {
                    model.addElement(f);
                }
            }
        }
    ...
    /** Ouvre l’élément sélectionné : dossier → naviguer, fichier .docx → charger. 
     * @throws Exception */
    private void openSelected() throws Exception {
        ...
        // Fichier .docx
        if (sel.isFile() && sel.getName().toLowerCase().endsWith(".docx")) {
            	readFile(sel);
                closeDialog(false);
        }
    }
    ...
    // ——— Recherche incrémentale avec priorité .docx > fichiers > dossiers ———
	    ...
	    if (list != null) {
	        for (File f : list) {
	            if (f.isDirectory()) {
	                ...
	                files++;
	                if (f.getName().toLowerCase().endsWith(".docx")) bweCount++;
	            }
	        }
	    }
	...
	private void showSelectedFileInfo() {
	    ...
	    // Ne traiter que les fichiers .docx
	    if (!f.isFile() || !f.getName().toLowerCase().endsWith(".docx")) {
	        return;
	    }
