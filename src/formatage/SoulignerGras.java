...
public class SoulignerGras {
	private final EditorApi ctx;

    public SoulignerGras(EditorApi ctx) {
        this.ctx = ctx;
    }
    ...
    public void appliquer() {
        ...
        try {
            ...
            // Aucune sélection ? on prend le mot sous le curseur
            if (start == end) {
            ...
            // Ne pas englober les espaces en bord
            ...
            if (hasPrefix && hasSuffix) {
                // --- Retirer _* ... *_ : suffixe d'abord, puis préfixe ---
                replace(doc, e, e + L, "");        // retire "*_"
                replace(doc, s - L, s, "");        // retire "_*"
                ...
                // Si tu préfères sélectionner seulement l'intérieur :
                // area.select(s + L, s + wrapped.length() - L);
            }
        } catch (Exception ex) {
            System.err.println("SoulignerGras _* *_ : " + ex.getMessage());
        }
    }
