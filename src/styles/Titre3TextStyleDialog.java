...
/**
 * Boîte de dialogue pour modifier les paramètres du style "Titre 3"
 * (node "Titre3" dans commandes.nodeblindWriter).
 *
 ...
 */
public final class Titre3TextStyleDialog {

    private Titre3TextStyleDialog() {
        // utilitaire static
    }

    /**
     * Ouvre la fenêtre de modification du style "Titre 3".
     * @param parent fenêtre parente (peut être null)
     */
    @SuppressWarnings("serial")
    public static void open(Window parent) {
        // Création du JDialog modal
        final JDialog dlg = new JDialog(parent, "Style : Titre 3", JDialog.ModalityType.APPLICATION_MODAL);
        ...
        var t3 = commandes.nodeblindWriter.retourneFirstEnfant("Titre3");
        ...
        String espBas       = "0.5cm";
        boolean keepNext    = true; // valeur par défaut : garder avec le suivant

        if (t3 != null) {
            String v;

            v = t3.getAttributs("police");
            if (v != null && !v.isBlank()) police = v.trim();

            v = t3.getAttributs("size");
            if (v != null && !v.isBlank()) size = v.trim();

            v = t3.getAttributs("alignement");
            if (v != null && !v.isBlank()) alignement = v.trim();

            v = t3.getAttributs("interligne");
            if (v != null && !v.isBlank()) interligne = v.trim();

            v = t3.getAttributs("espacement_au_dessus");
            if (v != null && !v.isBlank()) espHaut = v.trim();

            v = t3.getAttributs("espacement_en_dessous");
            if (v != null && !v.isBlank()) espBas = v.trim();

            v = t3.getAttributs("keep-with-next");
            if (v != null && !v.isBlank()) {
        ...
        btnSave.addActionListener(e -> {
            var node = commandes.nodeblindWriter.retourneFirstEnfant("Titre3");
            if (node != null) {
