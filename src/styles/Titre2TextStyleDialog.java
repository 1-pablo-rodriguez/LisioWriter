...
/**
 * Boîte de dialogue pour modifier les paramètres du style "Titre 2"
 * (node "Titre2" dans commandes.nodeblindWriter).
 *
 ...
 */
public final class Titre2TextStyleDialog {

    private Titre2TextStyleDialog() {
        // utilitaire static
    }

    /**
     * Ouvre la fenêtre de modification du style "Titre 2".
     * @param parent fenêtre parente (peut être null)
     */
    @SuppressWarnings("serial")
    public static void open(Window parent) {
        // Création du JDialog modal
        final JDialog dlg = new JDialog(parent, "Style : Titre 2", JDialog.ModalityType.APPLICATION_MODAL);
        ...
        var t2 = commandes.nodeblindWriter.retourneFirstEnfant("Titre2");
        ...
        String espBas       = "0.5cm";
        boolean keepNext    = true; // valeur par défaut : toujours garder avec le suivant

        if (t2 != null) {
            String v;

            v = t2.getAttributs("police");
            if (v != null && !v.isBlank()) police = v.trim();

            v = t2.getAttributs("size");
            if (v != null && !v.isBlank()) size = v.trim();

            v = t2.getAttributs("alignement");
            if (v != null && !v.isBlank()) alignement = v.trim();

            v = t2.getAttributs("interligne");
            if (v != null && !v.isBlank()) interligne = v.trim();

            v = t2.getAttributs("espacement_au_dessus");
            if (v != null && !v.isBlank()) espHaut = v.trim();

            v = t2.getAttributs("espacement_en_dessous");
            if (v != null && !v.isBlank()) espBas = v.trim();

            v = t2.getAttributs("keep-with-next");
            if (v != null && !v.isBlank()) {
        ...
        if (!foundAlign) {
            // On garde la valeur existante même si elle n'est pas dans la liste
        ...
        // Association label / composant pour lecteur d’écran
        ...
        // Ligne 7 : case "garder avec le suivant"
        ...
        btnSave.addActionListener(e -> {
            var node = commandes.nodeblindWriter.retourneFirstEnfant("Titre2");
            if (node != null) {
        ...
        // Bouton par défaut : Entrée déclenche Enregistrer
