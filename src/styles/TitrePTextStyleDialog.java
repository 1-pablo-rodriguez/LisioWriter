...
/**
 * Boîte de dialogue pour modifier les paramètres du style
 * "Titre principal" (node "Title" dans commandes.nodeblindWriter).
 *
 ...
 */
public final class TitrePTextStyleDialog {

    private TitrePTextStyleDialog() {
        // utilitaire static
    }

    /**
     * Ouvre la fenêtre de modification du style "Titre principal".
     * @param parent fenêtre parente (peut être null)
     */
    @SuppressWarnings("serial")
    public static void open(Window parent) {
        // Création du JDialog modal
        final JDialog dlg = new JDialog(parent, "Style : Titre principal", JDialog.ModalityType.APPLICATION_MODAL);
        ...
        var tprin = commandes.nodeblindWriter.retourneFirstEnfant("Title");

        String police       = "Arial";
        String size         = "18pt";
        String alignement   = "center";
        String interligne   = "150%";
        String espHaut      = "0.423cm";
        String espBas       = "0.212cm";
        boolean keepNext    = true; // garder avec le paragraphe suivant par défaut

        if (tprin != null) {
            String v;

            v = tprin.getAttributs("police");
            if (v != null && !v.isBlank()) police = v.trim();

            v = tprin.getAttributs("size");
            if (v != null && !v.isBlank()) size = v.trim();

            v = tprin.getAttributs("alignement");
            if (v != null && !v.isBlank()) alignement = v.trim();

            v = tprin.getAttributs("interligne");
            if (v != null && !v.isBlank()) interligne = v.trim();

            v = tprin.getAttributs("espacement_au_dessus");
            if (v != null && !v.isBlank()) espHaut = v.trim();

            v = tprin.getAttributs("espacement_en_dessous");
            if (v != null && !v.isBlank()) espBas = v.trim();

            v = tprin.getAttributs("keep-with-next");
            if (v != null && !v.isBlank()) {
        ...
        JLabel lblEspHaut = new JLabel("Espacement au-dessus (ex : 0.423cm) :");
        JLabel lblEspBas  = new JLabel("Espacement en dessous (ex : 0.212cm) :");
        ...
        btnSave.addActionListener(e -> {
            var node = commandes.nodeblindWriter.retourneFirstEnfant("Title");
            if (node != null) {
