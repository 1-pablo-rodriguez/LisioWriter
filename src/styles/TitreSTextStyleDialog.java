...
/**
 * Boîte de dialogue pour modifier les paramètres du style "Sous-titre"
 * (node "Subtitle" dans commandes.nodeblindWriter).
 *
 ...
 */
public final class TitreSTextStyleDialog {

    private TitreSTextStyleDialog() {
        // utilitaire static
    }

    /**
     * Ouvre la fenêtre de modification du style "Sous-titre".
     * @param parent fenêtre parente (peut être null)
     */
    @SuppressWarnings("serial")
    public static void open(Window parent) {
        // Création du JDialog modal
        final JDialog dlg = new JDialog(parent, "Style : Sous-titre", JDialog.ModalityType.APPLICATION_MODAL);
        ...
        var tstitre = commandes.nodeblindWriter.retourneFirstEnfant("Subtitle");
        ...
        if (tstitre != null) {
            ...
            v = tstitre.getAttributs("police");
            if (v != null && !v.isBlank()) police = v.trim();

            v = tstitre.getAttributs("size");
            if (v != null && !v.isBlank()) size = v.trim();

            v = tstitre.getAttributs("alignement");
            if (v != null && !v.isBlank()) alignement = v.trim();

            v = tstitre.getAttributs("interligne");
            if (v != null && !v.isBlank()) interligne = v.trim();

            v = tstitre.getAttributs("espacement_au_dessus");
            if (v != null && !v.isBlank()) espHaut = v.trim();

            v = tstitre.getAttributs("espacement_en_dessous");
            if (v != null && !v.isBlank()) espBas = v.trim();

            v = tstitre.getAttributs("keep-with-next");
            if (v != null && !v.isBlank()) {
        ...
        btnSave.addActionListener(e -> {
            var node = commandes.nodeblindWriter.retourneFirstEnfant("Subtitle");
            if (node != null) {
