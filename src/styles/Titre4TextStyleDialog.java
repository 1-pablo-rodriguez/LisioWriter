...
/**
 * Boîte de dialogue pour modifier les paramètres du style "Titre 4"
 * (node "Titre4" dans commandes.nodeblindWriter).
 *
 ...
 */
public final class Titre4TextStyleDialog {

    private Titre4TextStyleDialog() {
        // utilitaire static
    }

    /**
     * Ouvre la fenêtre de modification du style "Titre 4".
     * @param parent fenêtre parente (peut être null)
     */
    @SuppressWarnings("serial")
    public static void open(Window parent) {
        // Création du JDialog modal
        final JDialog dlg = new JDialog(parent, "Style : Titre 4", JDialog.ModalityType.APPLICATION_MODAL);
        ...
        var t4 = commandes.nodeblindWriter.retourneFirstEnfant("Titre4");
        ...
        if (t4 != null) {
            ...
            v = t4.getAttributs("police");
            if (v != null && !v.isBlank()) police = v.trim();

            v = t4.getAttributs("size");
            if (v != null && !v.isBlank()) size = v.trim();

            v = t4.getAttributs("alignement");
            if (v != null && !v.isBlank()) alignement = v.trim();

            v = t4.getAttributs("interligne");
            if (v != null && !v.isBlank()) interligne = v.trim();

            v = t4.getAttributs("espacement_au_dessus");
            if (v != null && !v.isBlank()) espHaut = v.trim();

            v = t4.getAttributs("espacement_en_dessous");
            if (v != null && !v.isBlank()) espBas = v.trim();

            v = t4.getAttributs("keep-with-next");
            if (v != null && !v.isBlank()) {
        ...
        btnSave.addActionListener(e -> {
            var node = commandes.nodeblindWriter.retourneFirstEnfant("Titre4");
            if (node != null) {
